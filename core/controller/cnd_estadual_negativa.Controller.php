<? 
	...
	if (empty($erro))
	{
		class Cnd_estadual_Negativa extends ADOdb_Active_Record
		{
			var $_table = 'cnd_estadual_negativa';
		}
		$obj = new Cnd_estadual_Negativa();
	...
	if (empty($id))	$obj->created = date('Y-m-d H:i:s') ;
		...
		try
		{
			$ok = $obj->save();  // esse save() vai retornar um UPDATE ou INSERT
             $sql =  "select *from cnd_estadual_negativa where espaco_fisico_id = '".$_SESSION["espaco_fisico_id"]."' order by id desc";
             $Rs = $DB->EXECUTE($sql);
            ...
            if (!empty($_FILES['cnd_estadual']['tmp_name']) and is_file($_FILES['cnd_estadual']['tmp_name'])) 
            {
                $caminho="../../upload/cnd_estadual_negativa/";
                $caminho = $caminho.$Rs->fields['id'].".pdf";
                /* Defina aqui o tipo de arquivo suportado */
                if ((eregi(".gif$", $_FILES['cnd_estadual']['name'])) || (eregi(".pdf", $_FILES['cnd_estadual']['name'])) || (eregi(".png", $_FILES['cnd_estadual']['name'])) || (eregi(".jpg$", $_FILES['cnd_estadual']['name'])) || (eregi(".txt$", $_FILES['cnd_estadual']['name'])))
                {
                    copy($_FILES['cnd_estadual']['tmp_name'],$caminho);
                } 
        	}  
