<? 
	...
	if (empty($erro))
	{
		class Crc_estado_piaui extends ADOdb_Active_Record
		{
			var $_table = 'crc_estado_piaui';
		}
		$obj = new Crc_estado_piaui();
	...
	if (empty($id))	$obj->created = date('Y-m-d H:i:s') ;
		...
		try
		{
			$ok = $obj->save();  // esse save() vai retornar um UPDATE ou INSERT
             $sql =  "select *from crc_estado_piaui where espaco_fisico_id = '".$_SESSION["espaco_fisico_id"]."' order by id desc";
             $Rs = $DB->EXECUTE($sql);
            if($_FILES['crc_estado_piaui']['size'] > 9024000)
            {
            ...
            if (!empty($_FILES['crc_estado_piaui']['tmp_name']) and is_file($_FILES['crc_estado_piaui']['tmp_name'])) 
            {
                $caminho="../../upload/crc_estado_piaui/";
                $caminho = $caminho.$Rs->fields['id'].".pdf";
                /* Defina aqui o tipo de arquivo suportado */
                if ((eregi(".gif$", $_FILES['crc_estado_piaui']['name'])) || (eregi(".pdf", $_FILES['crc_estado_piaui']['name'])) || (eregi(".png", $_FILES['crc_estado_piaui']['name'])) || (eregi(".jpg$", $_FILES['crc_estado_piaui']['name'])) || (eregi(".txt$", $_FILES['crc_estado_piaui']['name'])))
                {
                    copy($_FILES['crc_estado_piaui']['tmp_name'],$caminho);
                } 
        	}  
