	...
	if (empty($id))	$obj->created = date('Y-m-d H:i:s') ;
		...
		try
		{
			$ok = $obj->save();  // esse save() vai retornar um UPDATE ou INSERT
             $sql =  "select *from crc_sicaf where espaco_fisico_id = '".$_SESSION["espaco_fisico_id"]."' order by id desc";
             $Rs = $DB->EXECUTE($sql);
            if($_FILES['crc_sicaf']['size'] > 1024000)
            {
            ...
            if (!empty($_FILES['crc_sicaf']['tmp_name']) and is_file($_FILES['crc_sicaf']['tmp_name'])) 
            {
                $caminho="../../upload/crc_sicaf/";
                $caminho = $caminho.$Rs->fields['id'].".pdf";
                /* Defina aqui o tipo de arquivo suportado */
                if ((eregi(".gif$", $_FILES['crc_sicaf']['name'])) || (eregi(".pdf", $_FILES['crc_sicaf']['name'])) || (eregi(".png", $_FILES['cnd_estadual']['name'])) || (eregi(".jpg$", $_FILES['crc_sicaf']['name'])) || (eregi(".txt$", $_FILES['crc_sicaf']['name'])))
                {
                    copy($_FILES['crc_sicaf']['tmp_name'],$caminho);
                } 
        	}  
