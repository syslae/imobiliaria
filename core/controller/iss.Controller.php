  ...
  if (empty($id)) $obj->created = date('Y-m-d H:i:s') ;
    try
    {
        $ok = $obj->save();  // esse save() vai retornar um UPDATE ou INSERT
            $sql =  "select * from iss where  espaco_fisico_id = '".$_SESSION["espaco_fisico_id"]."' order by id desc";
            $Rs = $DB->EXECUTE($sql);
      if($_FILES['iss']['size'] > 2024000)
            {
            ...
            if (!empty($_FILES['iss']['tmp_name']) and is_file($_FILES['iss']['tmp_name']))
            {
                $caminho="../../upload/iss/";
                $caminho = $caminho.$Rs->fields['id'].".pdf";
                /* Defina aqui o tipo de arquivo suportado */
                if ((eregi(".gif$", $_FILES['iss']['name'])) || (eregi(".pdf", $_FILES['iss']['name'])) || (eregi(".png", $_FILES['iss']['name'])) || (eregi(".jpg$", $_FILES['iss']['name'])) || (eregi(".txt$", $_FILES['iss']['name'])))
                {
                    copy($_FILES['iss']['tmp_name'],$caminho);
                }
          }
