<?
    ...
    $situacao_geral = $_POST['situacao_geral'];
    
    if(empty($nosso_numero)){
		alert('Clique novamente na impressora da parcela.');
		redireciona('../ficha_financeira.php');
	}
    ...
    $SQL = "SELECT matricula as SeuNumero, (SELECT(LTRIM(RTRIM(STR(DAY(Data_Vencimento))))+'/'+RTRIM(LTRIM(STR(MONTH(Data_Vencimento))))+'/'+ LTRIM(RTRIM(STR(YEAR(Data_Vencimento)))))) AS vencimento,";
    $SQL = $SQL." idContas_a_Receber as NossoNumero, ROUND(valor,2) AS valor, matricula, '' as codCarteira, '' as codBarras, (SELECT(LTRIM(RTRIM(STR(DAY(Data_Abertura))))+'/'+rtrim(ltrim(str(month(Data_Abertura))))+'/'+ LTRIM(RTRIM(STR(YEAR(Data_Abertura)))))) AS dataEmissao,";
    $SQL = $SQL." idBanco, round(valor*0.0017,2) AS valorM, Mensagem1, Mensagem2, '' as Mensagem3, '' as Mensagem4, '' as Mensagem5, '' as Mensagem6, '' as Mensagem7,  Juros, '' as mensagem8, '' as mensagem9,  Multa, 0 as Moeda, 0 as Desconto FROM contas_a_receber WHERE 'ABERTO' = '".$situacao_geral."'";
    $SQL = $SQL." AND matricula = '".$mat."' AND idContas_a_Receber = ".$nosso_numero;
    ...
    //mensagens
    $multa_reais = $linha->fields['Multa'];
    if($multa_reais != ''){
        $multa_reais = ($multa_reais * $valorDoc)/100;
        $mensagemMulta = 'MULTA: <b>R$ '.arredondado($multa_reais,2).'</b>';    
    } 
    
    $juros_reais = $linha->fields['Juros'];
    if($juros_reais != ''){
        $juros_reais = ($juros_reais * $valorDoc)/100;
        $mensagemJuros = 'JUROS AO DIA: <b>R$ '.arredondado($juros_reais,2).'</b>';
    }
    ...
    elseif (strlen(substr($valorDoc,strpos($valorDoc,".") + 1,2)) == 2) $valorDoc = str_replace(".",",",$valorDoc);
    ...
    $SQL = "SELECT a.nome, a.Endereco, a.Bairro, a.Cidade, a.SiglaEstado, a.cep, (SELECT Descricao FROM Cursos WHERE a.idCurso = idCurso) as Descricao, a.idCurso, a.BlocoAtual, a.Numero, a.Complemento from alunos as a where matricula = '".$mat."'";
    $linha = $DB->Execute($SQL);
    ...
    switch ($idBanco)
    {
    	...
    	case "RS":
    		...
    		break;
    	case "BR":
    		$codCedente = $linha->fields['AgenciaCodCedenteBR'];
    		...
    		break;
    	case "SR":
    		$codCedente = $linha->fields['AgCodCedenteSIGCB'];
    		$localPagamento = "Pagável em qualquer banco, até o vencimento.";
    		$codBanco = "104";
    		$dvBanco = "0";
    		$especieDoc = "DM";
            ...
            include 'boleto_cef_sigcb.php';
            include 'boletophp/include/funcoes_cef_sigcb.php';

            $repNumerica = $dadosboleto["linha_digitavel"];
            $codBarra = $dadosboleto["codigo_barras"];
