...
$pdf->Cell ( $w[4], $h[1], 'Vencimento', 'LTR', 0, 'L', 0 );
$pdf->Ln();
...
$pdf->Cell ( $w[1], $h[1], '', 0, 0, 'L', 0 );
$pdf->Cell ( $w[3], $h[1], utf8_encode('Beneficiário'), 'LTR', 0, 'L', 0 );
$pdf->Cell ( $w[4], $h[1], utf8_encode('Agência/Código do Cedente'), 'LR', 0, 'L', 0 );
$pdf->Ln();
$pdf->Cell ( $w[0], $h[0], $vencimento, 'LBR', 0, 'L', 0 );
$pdf->Cell ( $w[1], $h[0], '', 0, 0, 'L', 0 );
$pdf->Cell ( $w[3], $h[0], utf8_encode($nomeCedente." - ". $CnpjAvalista), 'LBR', 0,'L', 0);
...
$pdf->Cell ( $w[1], $h[1], '', 0, 0, 'L', 0 );
$pdf->Cell ( $w[3], $h[0], utf8_encode('INSTRUÇÕES DE RESPONSABILIDADE DO BENEFICIÁRIO. '.$planoConta." ".$numeroParcela."/".$quantidadeParcela) , 'LR', 0,'L', 0);
$pdf->Cell ( $w[4], $h[1], '(-) Desconto/Abatimento', 'LTR', 0, 'L', 0 );
$pdf->Ln();
$pdf->Cell ( $w[0], $h[0], $numDoc, 'LBR', 0, 'L', 0 );
$pdf->Cell ( $w[1], $h[0], '', 0, 0, 'L', 0 );
$pdf->Cell ( $w[6], $h[0], utf8_encode($mensagemJuros." ".$instrucaoVenc1), 'L', 0,'L', 0);
$pdf->Cell ( $w[6], $h[0], $mensagemSequencial, 'R', 0,'L', 0);
...
$pdf->Cell ( $w[4], $h[1], utf8_encode('(-) Outras Deduções'), 'LTR', 0, 'L', 0 );
$pdf->Ln();
...
$pdf->Cell ( $w[4], $h[0], utf8_encode("CPF: ".$cpfSac." Bloco: ".$bloco) , 'TR', 0,'L', 0);
$pdf->Ln();
$pdf->Cell ( $w[0], $h[0], '', 'LBR', 0, 'L', 0 );
$pdf->Cell ( $w[1], $h[0], '', 0, 0, 'L', 0 );
...
$pdf->Cell ( $w[7]- ($w[4] * 2), $h[0], utf8_encode($Complemento." ".$cepSac ." ". $cidadeSac ." - ". $estadoSac) , 'L', 0,'L', 0);
$pdf->Cell ( $w[4], $h[0], utf8_encode("Turma:". $Turma) , 0, 0,'L', 0);
$pdf->Cell ( $w[4], $h[0], utf8_encode("Predio: ". $Predio) , 'R', 0,'L', 0);
$pdf->Ln();
$pdf->MultiCell($w[0], ($h[0]*2), utf8_encode(substr($nomeSac,0,28)).' ...','LBR','L');

$pdf->SetXY($pdf->GetX()+30, $pdf->GetY()-7);
$pdf->Cell ( $w[1], $h[0], '', 0, 0, 'L', 0 );
$pdf->Cell ( $w[7], $h[0], "Curso: ".utf8_encode($curso), 'LBR', 0,'L', 0);
$pdf->Ln();
