<? session_start();
	...
	{
		include(DOMAIN_PATH."core/atualizar/movimentacao.Atualizar.php");
	}
	else
	{
...
{	
	var qtd =$('#qtd').val();
	$('#filhos').append('<div id="filho'+ item["produto_id"]+'">Produto:'+ item["nome_produto"] +'<br>Quantidade:'+item["quantidade"]+'  Valor unitario: R$ '+item["valor_unitario"]+' Valor total: R$ '+item["valor_total"]+'<input type="hidden" name="produto_id[]" value="'+ item["produto_id"] +'" /><input type="hidden"  name="quantidade[]" value="'+ item["quantidade"] +'" /><input type="hidden"  name="valor_unitario[]" value="'+ item["valor_unitario"] +'" /><input type="hidden" name="valor_total[]" value="'+ item["valor_total"] +'" id="valor" "/><a href="javascript:;" onclick="javascript: excluir('+item["produto_id"]+','+item["produto_id"]+','+item["valor_total"]+')">  Deletar</a><hr></div>');
	qtd++;
	$('#qtd').val(qtd);
}
...
{
	$('#clientes_i').append('<input type="hidden" value="1" id="'+id+'p" />');
}
function remove_produto(id)
{
	$('#'+id+'p').remove();
