					...
					else
					{
						...
						while (!$rs->EOF)
						{
							...
							if (($_SESSION["permissao_temp"]>1))
							{
								...
								{
									$status = '<img id="img-'.$posicao.'" name="img-'.$posicao.'" src="../../webroot/img_sistema/bola-verde.gif" onclick="javascript:atualizaStatus('.$posicao.',0,'.$rs->fields['id'].',\'bancos\');"  border="0">';
								}
								else
								{
									$status = '<img id="img-'.$posicao.'" name="img-'.$posicao.'" src="../../webroot/img_sistema/bola-vermelha.gif" onclick="javascript:atualizaStatus('.$posicao.',1,'.$rs->fields['id'].',\'bancos\');"  border="0">';
								}
							}
							else
							{
							...
							echo '</td>
	                            ...
	                            <td class="text_padrao">'.$rs->fields['sigla'].'</td> 
	                            <td class="text_padrao">'.$rs->fields['juros'].'</td> 
	                            <td class="text_padrao">'.$rs->fields['multa'].'</td>
	                            <td class="text_padrao">'.$rs->fields['desconto'].'</td>
	                            <td class="text_padrao">'.$rs->fields['cnpj'].'</td>
	                            <td class="text_padrao">'.$rs->fields['exibir_parcela'].'</td>
	                            <td class="text_padrao">'.$rs->fields['nome_instituicao'].'</td>
