					...
					else
					{
						...
						while (!$rs->EOF)
						{
							...
							if (($_SESSION["permissao_temp"]>1))
							{
								...
								{
									$status = '<img id="img-'.$posicao.'" name="img-'.$posicao.'" src="../../webroot/img_sistema/bola-verde.gif" onclick="javascript:atualizaStatus('.$posicao.',0,'.$rs->fields['id'].',\'produto\');"  border="0">';
								}
								else
								{
									$status = '<img id="img-'.$posicao.'" name="img-'.$posicao.'" src="../../webroot/img_sistema/bola-vermelha.gif" onclick="javascript:atualizaStatus('.$posicao.',1,'.$rs->fields['id'].',\'produto\');"  border="0">';
								}
							}
							else
							{
							...
							echo '</td>
	                            ...
	                            <td class="text_padrao">'.formata_data($rs->fields['created']).'</td>
	                            <td align="center"><div id="div-'.$posicao.'">'.$status.'</div></td>
	                   		    <td align="left">'; botaoCheckbox($rs->fields['id']);echo '&nbsp;</td>
