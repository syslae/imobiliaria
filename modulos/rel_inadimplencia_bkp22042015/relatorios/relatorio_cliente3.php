...
//Instancia da Classe
    ...
    $sql2 = "select c.nome as nome, c.cpf as cpf, p.parcela as parcela, p.valor as valor, p.data_vencimento as vencimento from parcelas p
            ...
            where p.situacao_pagamento_id = 1 $Condicao";
 	// $sql2 = "select descricao, valor, codigo from produto 
  //      where espaco_fisico_id = '".$_SESSION['espaco_fisico_id']."' and produto_principal_id is not null $Condicao";
	$rs = $DB->Execute($sql2);
	$pdf->AddPage();
	$pdf->SetWidths(array(100,40,40));	
    $pdf->Row(array("Descrição","Valor","Código"),array("L","L","L"));
    $pdf->SetFont('Arial','',8);
    ...
    while (!$rs->EOF)
	{
     ...
     $pdf->Row(array("$descricao","$valor","$codigo "),array("L","L","L"));				
