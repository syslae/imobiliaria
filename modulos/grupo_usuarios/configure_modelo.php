<? session_start();
	...
	require("../Genesis/define.php");
	require(DOMAIN_PATH."Genesis/conexaoAr.php");
	require(DOMAIN_PATH."funcoes.inc.php");
	require(DOMAIN_PATH."class_anti_injection.php");

	if(empty($_SESSION["idwebsite_g"]))
		redireciona('../');
	...
	$campos = "id, login, senha, nome, email, tipo";
	###        CONFIGURACAO        ###
	...
	*/
	...
	if(!empty($secaoPermissao))
	{
		...
		else if($secaoPermissao == '250f688520b14472f475e7bc72d873b454f4f60e')
		{
			...
			$sqlp="delete from usuariotabelas where usuario_id = $id and website_id = $_SESSION[idwebsite_g]";
			$DB->Execute($sqlp);
			
			$sqlp="delete from usuariosecoes where usuario_id = $id and website_id = $_SESSION[idwebsite_g]";
			$DB->Execute($sqlp);
...
<?php
/* -----------------------------------------------------------------------------------------------------------------------------
* Criado em: 13-10-2008 16:05:12
* Site: Brasilportais
* Autor: Rupert Brasil Lustosa 
- rupertbrasil@gmail.com
* Graduado em Tecnologia em Informática pelo CEFET-PI 2005/2
------------------------------------------------------------------------------------------------------------------------------*/	
...
?>
                ...
                <table width="830" border="0" cellspacing="0" cellpadding="2" style="margin-top:20px;">
					<tr>
                    	<td width="129" align="center" class="text_bold_preto">Atalhos</td>
                        <td width="129" align="center" class="text_bold_preto">Websites</td>
                        <?
                        if($rs->fields['tipo']==0)
						{
						?>
                        <td width="129" align="center" class="text_bold_preto">Administra&ccedil;&atilde;o</td>
                        <?=$tdTitulo?>
                        <?
                        }
						?>
                ...
                <table width="830" border="0" cellspacing="0" cellpadding="2" style="margin-top:20px;">
					...
					<? 	if($rs->fields['tipo']==0) { ?>
                    <tr>
					  <td class="text_bold_preto" align="center"><a href="configure.php?<?=$linkbase.geraSenha('0c')?>">Remover todas as permiss&otilde;es desse usu&aacute;rio desse Website</a></td>
				    </tr>
                    <? } ?>
					<tr>
                        ...
                        <table width="444" border="0" cellspacing="4" cellpadding="2">
                            <tr class="text_titulo-roxo">
							  ...
							  // se for diferente de 8 eu faco a pesquisa em secoes, se for = 8 faco a pesquisa em tabelas
								if($menu_id == '0a')
								{
									$sql="select * from websites order by nome asc";
									$tabelapermissoes = 'usuariowebsites';
									$tabelaPermissoesInt = 3;
									$campoPermissao = 'website_id';
								}
								else if($menu_id == "0b")
								{
								...
								{
									$sql="select s.* from secoes s, websitesecoes ws where s.status = 1 and s.menu_id = $menu_id and s.id = ws.secao_id order by s.nome asc";
									$tabelapermissoes = 'usuariosecoes';
									$tabelaPermissoesInt = 1;
									$campoPermissao = 'secao_id';
								}
                              ...
                              while (!$linha->EOF)
                              {
								...
								if($tabelaPermissoesInt==3) // sendo 3 nao vejo o website porque a permissao para website é algo geral dada apenas pelo 180
	                                $sqlp="select * from $tabelapermissoes where $campoPermissao=$idt and usuario_id=$id";
								else $sqlp="select * from $tabelapermissoes where $campoPermissao=$idt and usuario_id=$id and website_id = ".$_SESSION["idwebsite_g"]."";
                                $linhap = $DB->Execute($sqlp);
                              ...
                              <tr>
                                ...
                                <td>
                                    <div id="status<?=$idt?>">
                                        <?
                                       	// se a $tabelaPermissoesInt = 3 mostro apenas duas opcoes. Tem acesso ou nao tem acessso ao website
